...
public class IntelligentTextWatcher implements TextWatcher {
    ...
    public IntelligentTextWatcher(int editTextId, TextWatcherAdapterInterface textWatcherAdapterInterface) {
                ...
                try {
                    if (count >= 2) {//表情符号的字符长度最小为2
                        ...
                        if (containsEmoji(input.toString())) {
                            resetText = true;
                            Toast.makeText(BaseApplication.getContext(), "不支持输入Emoji表情符号", Toast.LENGTH_SHORT).show();
